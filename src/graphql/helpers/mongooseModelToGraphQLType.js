...
function mongooseTypeToGraphQLType(name, pathObj, level = 0, isInput) {
  ...
  if (pathObj.instance === 'ObjectID' && pathObj.options.ref && level <= MAX_LEVEL && !isInput) {
    const model = models[capitalize(pathObj.options.ref)];
    const typeName = `${capitalize(name)}View${capitalize(pathObj.path)}`;
    return mongooseModelToGraphQLType(typeName, model, {}, level + 1);
  }
  if (pathObj.instance === 'String' && pathObj.options.enum) {
  ...
  const options = pathObj.options.graphQLOptions || {};
  const typesMap = {
    'ObjectID': GraphQLID,
...
function mongooseFieldToGraphQLField(name, field, level = 0, isInput = false) {
  check.assert.assigned(name, '"name" is required');
  check.assert.object(field, '"field" should be object');
  let type = mongooseTypeToGraphQLType(name, field, level, isInput);
  if (field.options.required) {
    type = new GraphQLNonNull(type);
  }
  const fld = {
    type,
    filter: field.instance !== 'Embedded' && (field.options.graphQLOptions && field.options.graphQLOptions.filter !== false),
    sort: true,
    textSearch: field.options.graphQLOptions && field.options.graphQLOptions.textSearch
  };
  if (field.options.ref && !isInput) {
    fld.foreign = true;
    const Service = getServiceForModel(field.options.ref);
    fld.resolve = async (parent) => {
      const service = new Service();
      const val = parent[field.path];
...
function mongooseSchemaToGraphQLInputType(name, schema) {
  ...
  paths.forEach((f) => {
    ...
    const options = path.options.graphQLOptions || {};
    if (options.hidden) {
      return;
    }
  ...
  if (level === 0) {
    ...
    Object.keys(refs).forEach((refName) => {
      ...
      ref.forEach((refField) => {
        ...
        const getContext = (parent) => {
          return {[refField]: parent._id};
        };
